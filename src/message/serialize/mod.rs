...
impl<VT> ViewTransferProtocolMessage for ViewTransfer<VT>
    ...
    fn internally_verify_message<NI>(network_info: &Arc<NI>, header: &Header, message: &Self::ProtocolMessage) -> atlas_common::error::Result<()> where NI: NetworkInformationProvider {
        Ok(())
    }
}
